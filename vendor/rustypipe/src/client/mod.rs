...
pub enum ClientType {
    ...
    /// - no obfuscated stream URLs
    Ios,
    /// Client used by the visionOS app
    ///
    /// - no obfuscated stream URLs
    /// - sirve streams de googlevideo sin el techo posicional (~1MiB) que
    ///   YouTube aplica a ANDROID_VR/iOS desde 2026
    /// - exige PO token para descargar streams HTTPS
    Visionos,
}
...
impl ClientType {
    ...
    fn needs_deobf(self) -> bool {
        // parche PlayFusion: el cliente Android (como yt-dlp ANDROID) devuelve
        // URLs directas sin firma, así que no necesita deofuscación.
        matches!(self, ClientType::Desktop | ClientType::DesktopMusic)
    }

    fn needs_po_token(self) -> bool {
        // parche PlayFusion: YouTube exige PO tokens también en Android/iOS
        // (2026); sin ellos los streams responden 403 Forbidden. VISIONOS los
        // requiere para streams HTTPS (política GVS) y además es el único
        // cliente directo actual sin techo posicional en googlevideo.
        matches!(
            self,
            ClientType::Desktop
...
pub(crate) const YOUTUBE_HOME_URL: &str = "https://www.youtube.com";
...
const ANDROID_CLIENT_VERSION: &str = "1.65.10";
const ANDROID_VERSION: &str = "12L";
const ANDROID_DEVICE_MAKE: &str = "Oculus";
const ANDROID_DEVICE_MODEL: &str = "Quest 3";

// VisionOS client (parche PlayFusion): parámetros de contexto públicos del
// cliente VISIONOS (client id 101). Sirve streams sin el techo posicional.
const VISIONOS_CLIENT_VERSION: &str = "1.02";
const VISIONOS_OS_VERSION: &str = "26.5.23O471";
const VISIONOS_DEVICE_MAKE: &str = "Apple";
const VISIONOS_DEVICE_MODEL: &str = "RealityDevice17,1";
/// User-Agent del cliente visionOS.
pub const VISIONOS_UA: &str = "Mozilla/5.0 (Macintosh; Intel Mac OS X 15_7_3) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/26.0 Safari/605.1.15";
...
impl RustyPipeQuery {
    ...
    pub fn user_agent(&self, ctype: ClientType) -> Cow<'_, str> {
        match ctype {
            ...
            .into(),
            ClientType::Visionos => VISIONOS_UA.into(),
        }
    }
        ...
        match ctype {
            ...
            ClientType::Android => YTContext {
                client: ClientInfo {
                    ...
                    device_make: ANDROID_DEVICE_MAKE,
            ...
            ClientType::Visionos => YTContext {
                client: ClientInfo {
                    client_name: "VISIONOS",
                    client_version: VISIONOS_CLIENT_VERSION.into(),
                    device_make: VISIONOS_DEVICE_MAKE,
                    device_model: VISIONOS_DEVICE_MODEL,
                    os_name: "visionOS",
                    os_version: VISIONOS_OS_VERSION,
                    platform: "DESKTOP",
                    visitor_data,
                    hl,
                    gl,
    ...
    async fn request_builder(
        ...
        let mut r = match ctype {
            ...
            ClientType::Ios => self
                ...
                .header("X-Goog-Api-Format-Version", "2"),
            // VisionOS (parche PlayFusion): endpoint estándar de youtubei con
            // el client id 101 del contexto VISIONOS.
            ClientType::Visionos => self
                .client
                .inner
                .http
                .post(format!(
