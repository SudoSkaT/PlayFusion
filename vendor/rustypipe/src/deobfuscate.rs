...
c[36](c[8],c[32]),c[20](c[25],c[10]),c[2](c[22],c[8]),c[32](c[20],c[16]),c[32](c[47],c[49]),c[1](c[44],c[28]),c[39](c[16]),c[32](c[42],c[22]),c[46](c[14],c[48]),c[26](c[29],c[10]),c[46](c[9],c[3]),c[32](c[45])}catch(d){return"enhanced_except_85UBjOr-_w8_"+a}return b.join("")};var deobf_nsig=Vo;"#;
    ...
    /// Reproduce la extracción contra la player.js ACTUAL (requiere red) para
    /// ver qué patrón de la player.js de 2026 falla.
    #[tokio::test]
    #[ignore]
    async fn t_extract_current_player_js() {
        let client = Client::new();
        match get_player_js_url(&client).await {
            Ok(url) => {
                println!("player_js url: {url}");
                let js = get_response(&client, &url).await.unwrap();
                println!("player_js len: {}", js.len());
                match DeobfData::extract_fns(&url, &js) {
                    Ok(d) => println!(
                        "EXTRACCION OK: sig={} nsig={} sts={}",
                        d.sig_fn.len(),
                        d.nsig_fn.len(),
                        d.sts
                    ),
                    Err(e) => println!("EXTRACCION FALLA: {e}"),
                }
            }
            Err(e) => println!("no player_js url: {e}"),
        }
    }
